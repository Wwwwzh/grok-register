...
# Updated 2026-07-15 from live accounts.x.ai chunk 0n02b5znnl0t_.js
...
class ProtocolOAuthClient:
    ...
    def login(
        ...
        *,
        ...
        def _resolve_consent_action_id(page_html: str = "") -> str:
            """Resolve live submitOAuth2Consent action id from HTML or linked JS chunks."""
            m = re.search(
            ...
            if m:
                return m.group(1)
            # Action id usually lives in a Next.js chunk, not the HTML itself.
            scripts = re.findall(r'src="([^"]+_next/static/[^"]+\.js)"', page_html or "")
            for src in scripts[:50]:
                url = src if src.startswith("http") else urljoin(ACCOUNTS_ORIGIN, src)
                try:
                    js = self._s.get(url, timeout=30).text or ""
                except Exception:
                    continue
                if "submitOAuth2Consent" not in js:
                    continue
                m = re.search(
                    r'createServerReference\)\("([a-f0-9]{40,44})"[^)]*submitOAuth2Consent',
                    js,
                ...
                if m:
                    self._log(f"resolved consent action from chunk {url.rsplit('/', 1)[-1]}")
                    return m.group(1)
        ...
        def _submit_oauth2_consent(page_url: str, page_html: str = "") -> str:
            ...
            action_id = _resolve_consent_action_id(page_html)
